...
/**
 *
 * @author tomoaki 從bpm copy 過來 一直以來在想如何解決 Response 時, 對方回傳有錯之問題.因此之前建了這居
 *
 * 目前在改名 WebServiceResponseException
 *
 * 其實之前有類似的（難怪想說為啥以前明明有傳進Response卻找不到......之前的是叫JerseyResponseException 在
 * CardLogManagement底下
 *
 */
public class WebServiceResponseException extends RuntimeException {
    ...
    public static class Factory {
        ...
        public static WebServiceResponseException create(WebTarget webTarget) {
            String msg = String.format("Error Occur When Create connection To %s", webTarget.getUri());
            return WebServiceResponseException.Factory.create(msg);
        }
        

        public static WebServiceResponseException create(WebTarget webTarget, Exception ex) {
            String msg = String.format("Exception Occur Before Connect To %s, Error Msg Is= %s", webTarget.getUri(), ex.getMessage());
            return WebServiceResponseException.Factory.create(msg);
        }        
