...
//        System.out.println(dataStreamSupplier.get().limit(limitSize).collect(Collectors.toList()));
    ...
    protected static List test5(List fromDataList, List removingDataList) {
        return DataCollectionUtils.removeFrom(fromDataList, removingDataList);
    }

    protected static void test6() {
        // System.out.println(test4(Stream.of(1, 3, 7, 31, 0, -1, 23).collect(Collectors.toList()), 1, 2));
        System.out.println(test5(Stream.of(1, 3, 7, 31, 0, -1, 23).collect(Collectors.toList()), Stream.of(7, 32, 15, -1, 1).collect(Collectors.toList())));
    }
    
    protected static void test7() {
        List<Integer> dataList = DataCollectionUtils.preordering(Arrays.asList(1, 3, 5, 7, 9), 7, 9); // DataCollectionUtils.preordering(Arrays.asList(1, 3, 5, 7, 9), Arrays.asList(7, 9));
